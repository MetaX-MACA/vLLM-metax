...
def get_maca_version() -> tuple[Version, str] | None:
    ...
    maca_version = release_version_str.split("-")[0]  # Get the version part before the dash
    tag = release_version_str.split("-")[-1] if "-" in release_version_str else None  # Get the tag part after the dash, default to "None" if not present
