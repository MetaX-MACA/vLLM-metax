...
from vllm_metax.ops.fused_moe import (
    MacaUnquantizedFusedMoEMethod
)
...
class MacaCompressedTensorsMoEMethod(CompressedTensorsMoEMethod):
    ...
    def get_moe_method(
        ...
        if should_ignore_layer(prefix, quant_config.ignore, quant_config.packed_modules_mapping):
        ...
        if quant_config._is_wNa16_group_channel(weight_quant, input_quant):
            ...
            if not check_moe_marlin_supports_layer(layer, group_size):
                ...
                raise RuntimeError(
                    f"Unsupported FusedMoe scheme: CompressedTensorsWNA16MarlinMoEMethod")
        elif quant_config._is_fp4a4_nvfp4(weight_quant, input_quant):
            return MacaCompressedTensorsWNA16MoEMethod(layer.moe_config)
        elif quant_config._is_dynamic_token_w8a8(weight_quant, input_quant):
            return MacaCompressedTensorsW8A8Int8MoEMethod(quant_config,
                                                      layer.moe_config)
        elif quant_config._is_dynamic_token_w4a8_int(weight_quant,
                                                     input_quant):
            logger.info_once("Using MacaCompressedTensorsW4A8Int4MoEMethod")
            return MacaCompressedTensorsW4A8Int4MoEMethod(quant_config,
                                                      layer.moe_config, matched_target)
        else:
            raise RuntimeError(
...
class MacaCompressedTensorsW4A8Int4MoEMethod(CompressedTensorsMoEMethod):
    ...
    def __init__(
        ...
        self.target = "Linear"
        if matched_target is not "":
            self.target = matched_target

        self.weight_quant = self.quant_config.target_scheme_map[self.target].get("weights")
        self.input_quant = self.quant_config.target_scheme_map[self.target].get("input_activations")

        # Must be dynamic per-token activations
        if self.input_quant.strategy != QuantizationStrategy.TOKEN or not self.input_quant.dynamic:
            raise ValueError(
    ...
    def create_weights(self, layer: torch.nn.Module, num_experts: int,
        ...
        w2_weight_scale = torch.nn.Parameter(torch.ones(num_experts,
                                                        hidden_size,
                                                        intermediate_size_per_partition // self.group_size if self.group_size else 1,
                                                        dtype=torch.float32),
                                                        requires_grad=False)
