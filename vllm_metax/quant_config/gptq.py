...
class GPTQLinearMethod(vllm_GPTQLinearMethod):
            ...
            if layer.scales.dtype != torch.bfloat16:
                ...
                if self.quant_config.weight_bits == 4:
                    ...
                    _ = ops.gptq_gemm(
                        ...
                        self.quant_config.weight_bits,
                        self.quant_config.group_size, perm_space, temp_space,
                        False)
    ...
    def apply(self,
        ...
        return torch.ops.vllm._apply_gptq(x, qweight, scales, qzeros, bias,
                                          g_idx, use_exllama, weight_bits,
                                          group_size, desc_act)

    # └------------------------- Metax Modification -------------------------┘


def _apply_gptq_fake(x: torch.Tensor, qweight: torch.Tensor,
                     scales: torch.Tensor, qzeros: torch.Tensor,
                     bias: torch.Tensor, g_idx: torch.Tensor,
                     use_exllama: bool, weight_bits: int, group_size: int,
                     desc_act: bool) -> torch.Tensor:
    out_shape = x.shape[:-1] + (qweight.shape[-1], )
    return torch.empty(out_shape, dtype=x.dtype, device=x.device)


def _apply_gptq(x: torch.Tensor, qweight: torch.Tensor, scales: torch.Tensor,
                qzeros: torch.Tensor, bias: torch.Tensor, g_idx: torch.Tensor,
                use_exllama: bool, weight_bits: int, group_size: int,
                desc_act: bool) -> torch.Tensor:
    ...
    if weight_bits == 4 or weight_bits == 8 or group_size == 128 or group_size == 64:
        if desc_act:
            perm_space = torch.empty(reshaped_x.shape[0],
                                     reshaped_x.shape[1],
                                     dtype=torch.float16,
                                     device=x.device)
    ...
    output = ops.gptq_gemm(reshaped_x, qweight, qzeros, scales, g_idx,
                           use_exllama, weight_bits, group_size, perm_space,
