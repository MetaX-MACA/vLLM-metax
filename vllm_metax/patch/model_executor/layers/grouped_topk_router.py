...
def fused_grouped_topk(
    ...
    if scoring_func == "sigmoid":
        ...
        # ┌------------------------  Metax Modification -------------------------┐
        deepseek = (topk == 8 and num_expert_group == 8 and topk_group == 4) or (topk == 9 and num_expert_group == 8 and topk_group == 4)
        kimi = (topk == 8 and num_expert_group == 1 and topk_group == 1) or (topk == 9 and num_expert_group == 1 and topk_group == 1)
        if mx_envs.VLLM_METAX_USE_SGL_FUSED_MOE_GROUPED_TOPK and (gating_output.shape[1] // num_expert_group <= 384) and (kimi or deepseek):
            bias_bf16 = e_score_correction_bias.to(torch.bfloat16)
            topk_values = torch.empty((hidden_states.shape[0], topk), dtype=torch.float, device=hidden_states.device)
            topk_indices = torch.empty((hidden_states.shape[0], topk), dtype=torch.int, device=hidden_states.device)
