...
class IpcBuffer:
    ...
    def __init__(self, rank: int, world_size: int, size: int, process_group=None):
        ...
        self.local_ptr, _ = _mc_malloc_fine_grained(size)
        _cuda_memset_zero(self.local_ptr, size)
        self._alive = True

        # --- exchange IPC handles via torch.distributed ---
        local_handle = libcudart.cudaIpcGetMemHandle(self.local_ptr)

        all_handles: list[bytes | None] = [None] * world_size
        torch.distributed.all_gather_object(
            all_handles, bytes(local_handle.internal), group=process_group
        )
...
# lamport_workspace.IpcBuffer = IpcBuffer
# lamport_workspace._cuda_memcpy_d2d = _cuda_memcpy_d2d

# [TODO] The following is untouched
# [TODO] To be enabled after maca-python-as-cuda support
