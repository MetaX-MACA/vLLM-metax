...
class MacaEagleProposer(EagleProposer):
    ...
    def propose(
        ...
        num_tokens_dp_padded, batch_size_across_dp = self._pad_batch_across_dp(
            batch_size, input_batch_size
        )
        input_batch_size = num_tokens_dp_padded
        ...
        for fwd_idx in range(
            ...
            if fwd_idx <= 1:
                ...
                if (
                    ...
                    num_tokens_after_pad = self.vllm_config.pad_for_cudagraph(
                        num_tokens
                    )
                else:
                    num_tokens_after_pad = num_tokens

                num_tokens_dp_padded, num_tokens_across_dp = self._pad_batch_across_dp(
                    num_tokens, num_tokens_after_pad
                )
                num_input_tokens = num_tokens_dp_padded
                ...
                if (
                    ...
                    cudagraphs_enabled_mode = True
                else:
                    cudagraphs_enabled_mode = False
            # \------------------------- Metax Modification -------------------------/
