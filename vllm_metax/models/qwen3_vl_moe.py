...
class Qwen3VLMoeForConditionalGeneration(
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        with self._mark_language_model(vllm_config):
            lm_architectures = None

            # Qwen3-VL-MoE stores the language model config in `text_config`.
            # Some configs do not define `text_config.architectures`. This is only
            # required when PP is enabled, because vLLM validates whether the inner
            # language model supports pipeline parallelism.
            if (
                vllm_config.parallel_config.pipeline_parallel_size > 1
                and not getattr(config.text_config, "architectures", None)
            ):
                lm_architectures = ["Qwen3MoeForCausalLM"]

            self.language_model = Qwen3MoeLLMForCausalLM(
                vllm_config=vllm_config.with_hf_config(
                    config.text_config,
                    architectures=lm_architectures,
                ),
