...
class FlashMLASparseMetadataBuilder(
    ...
    def __init__(
        ...
        # MetaX's sparse decode kernels use a distinct path from prefill, so
        # only single-token queries (plus speculative tokens) are decodes.
    ...
    def _build_bf16_separate_prefill_decode(
        ...
        if num_decodes > 0:
            ...
            decode_query_len = (query_start_loc_cpu[1] - query_start_loc_cpu[0]).item()
            assert decode_query_len > 0
            # Full CUDA graphs include zero-token padding requests in
            # metadata.num_decodes. FlashMLA must only see active requests.
            active_num_decodes = num_decode_tokens // decode_query_len
            assert active_num_decodes * decode_query_len == num_decode_tokens
    ...
    def build(
        ...
        metadata.bf16_use_mixed_batch = False

        if self.use_fp8_kv_cache:
            metadata.fp8_use_mixed_batch = self.num_heads < MIN_HEADS_FOR_BF16_PREFILL
            ...
            # Unlike upstream, MetaX has distinct BF16 prefill and decode
            # kernels. Always build metadata for the separated path.
...
class FlashMLASparseImpl(SparseMLACommonImpl[FlashMLASparseMetadata]):
    ...
    def _forward_bf16_kv_separate_prefill_decode(
        ...
        # forward_mqa may receive only decodes when short prefills are handled
        # by dense MHA, or the complete batch when sparse MQA handles prefills.
        ...
        if num_decode_tokens > 0 and num_prefill_tokens == 0:
            assert bf16_metadata.decode is not None
            attn_out = _bf16_decode(q, topk_indices)
        else:
            ...
            attn_out[num_decode_tokens:num_mqa_tokens] = self._bf16_flash_mla_kernel(
                q[num_decode_tokens:num_mqa_tokens],
                kv_c_and_k_pe_cache,
                topk_indices[num_decode_tokens:num_mqa_tokens],
                topk_length=topk_length[num_decode_tokens:num_mqa_tokens],
            )
