...
class MacaFlashMLASparseBackend(AttentionBackend):
    ...
    supported_kv_cache_dtypes: ClassVar[list[CacheDType]] = [
        ...
        # "int8",  # to be added in future when int8 support is ready
    ]
...
class FlashMLASparseMetadata(AttentionMetadata):
    ...
    @dataclass
    class BF16SeparatePrefillDecode:
        @dataclass
        class Decode:
            seq_lens: torch.Tensor
            kernel_metadata: "FlashMLASparseMetadata.BF16KernelMetadata"
            decode_query_len: int  # needed for reshape in spec decode
    ...
    bf16_extra_metadata: BF16SeparatePrefillDecode | BF16KernelMetadata | None = None
    bf16_use_mixed_batch: bool = False
...
class FlashMLASparseMetadataBuilder(
    ...
    def __init__(
        ...
        self.use_bf16_kv_cache = cache_config.cache_dtype in ["auto", "bfloat16"]
        # self.use_int8_kv_cache = cache_config.cache_dtype == "int8"
...
class FlashMLASparseImpl(SparseMLACommonImpl[FlashMLASparseMetadata]):
    ...
    def _forward_bf16_kv_separate_prefill_decode(
        self,
        q: torch.Tensor,
        kv_c_and_k_pe_cache: torch.Tensor,
        topk_indices: torch.Tensor,
        ...
        """Use the paged decode kernel and the flat BF16 prefill kernel.

        As in the FP8 split path, q contains either the decode prefix or all
        tokens; prefills handled by dense MHA are absent. Unlike FP8 prefills,
        BF16 prefills read the cache directly through a zero-copy flat view.
        Decode indices encode (page, offset), while prefill indices must use
        the physical row stride when cache pages are not contiguous.

        actual_num_heads excludes any padding already in q_concat_buffer.
        Both kernels must trim that padding before returning or merging outputs.
        """
        ...
        if num_decode_tokens > 0:
            # The paged kernel handles cache strides; decode indices encode
            # page * block_size + offset. Preserve the original request-local
            # topk_indices for the separate prefill physical-row conversion.
            decode_indices = triton_convert_req_index_to_global_index(
        ...
        def _bf16_decode(q: torch.Tensor, topk_indices: torch.Tensor) -> torch.Tensor:
            ...
            attn_out, _ = self._bf16_flash_mla_kernel_with_kvcache(
                q=q,
                kv_c_and_k_pe_cache=kv_c_and_k_pe_cache,
                topk_indices=topk_indices,
                kernel_metadata=bf16_metadata.decode.kernel_metadata,
        ...
        if num_decode_tokens > 0 and num_prefill_tokens == 0:
            ...
            attn_out = _bf16_decode(q, decode_indices)
        else:
            # Mixed or pure prefill: allocate output tensor
            attn_out = q.new_empty(
                (num_mqa_tokens, actual_num_heads, self.kv_lora_rank),
                dtype=q.dtype,
                device=q.device,
            )
            ...
            if num_decode_tokens > 0:
                ...
                attn_out[:num_decode_tokens] = _bf16_decode(
                    q[:num_decode_tokens], decode_indices
                )

            if num_prefill_tokens > 0:
                # Keep the cache in place: flattening the original paged
                # tensor with view/reshape can copy the entire cache.
                kv_rows, block_stride_rows = flat_kv_row_view(
                    kv_c_and_k_pe_cache, attn_metadata.block_size
                )
                prefill_indices, topk_length = triton_convert_req_index_to_global_index(
                    attn_metadata.req_id_per_token[num_decode_tokens:num_mqa_tokens],
                    attn_metadata.block_table,
                    topk_indices[num_decode_tokens:],
                    BLOCK_SIZE=attn_metadata.block_size,
                    BLOCK_STRIDE_ROWS=block_stride_rows,
                ...
                prefill_out, _ = self._bf16_flash_mla_kernel(
                    q[num_decode_tokens:],
                    kv_rows,
                    prefill_indices,
    ...
    # This used bf16 prefill kernel (for mixed batch)
    ...
    def _bf16_flash_mla_kernel_with_kvcache(
        ...
        # q shape: (batch, seq_len, num_heads, head_dim)
        padded_num_heads = self.bf16_decode_padded_heads

        # Pad query if needed (kernel only supports h_q = 64 or 128)
        if q.size(2) < padded_num_heads:
            logger.warning_once(
                f"Padding num_heads from {actual_num_heads} to "
                f"{padded_num_heads} for BF16 sparse decode kernel"
            )
            q_padded = q.new_zeros((q.size(0), q.size(1), padded_num_heads, q.size(3)))
            q_padded[:, :, :actual_num_heads, :] = q
            q = q_padded
        ...
        attn_out = attn_out[:, :, :actual_num_heads, :]
        lse = lse[:, :actual_num_heads, :]
        return attn_out, lse
    ...
    def forward_mqa(
        ...
        use_bf16_cache = self.kv_cache_dtype in ["auto", "bfloat16"]

        lse: torch.Tensor | None = None

        if use_bf16_cache:
            if attn_metadata.bf16_use_mixed_batch:
                ...
                attn_out = self._forward_bf16_kv_separate_prefill_decode(
                    q,
                    kv_c_and_k_pe_cache,
                    topk_indices,
                    attn_metadata,
            ...
            if attn_metadata.fp8_use_mixed_batch:
