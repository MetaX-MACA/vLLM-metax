...
def dsa_indexer_uses_fp4(vllm_config: VllmConfig) -> bool:
    ...
    if use_fp4:
        raise ValueError("indexer_kv_dtype='mxfp4' requires higher compute capability.")
    return use_fp4
...
class MacaDeepseekV4IndexerBackend(MacaDeepseekV32IndexerBackend):
    @staticmethod
    def get_name() -> str:
...
def _supports_native_decode(next_n: int) -> bool:
    ...
    """
    # TODO(hank): need further checking on this, currently disable.
        ...
        if num_decodes > 0:
            ...
            if has_deep_gemm():
                metadata = get_paged_mqa_logits_metadata(
                    ...
                    self.kv_cache_spec.storage_block_size,
                    self.num_sms,
                    indices=decode_indices,  # maca deep_gemm does not support this yet
                )
