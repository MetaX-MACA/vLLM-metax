...
class MacaFlashInferBackend(AttentionBackend):
    ...
    def get_fp8_dtype_for_flashinfer(kv_cache_dtype: str) -> torch.dtype:
        raise NotImplementedError("Maca does not support FP8 FlashInfer.")
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        if self.cache_dtype.startswith("fp8"):
            self.kv_cache_dtype = MacaFlashInferBackend.get_fp8_dtype_for_flashinfer(
                self.cache_dtype
            )
        else:
    ...
    # /------------------------  Metax Modification -------------------------\
    # Note: MetaX won't support cascade attention for now
    # \------------------------  Metax Modification -------------------------/
    def _get_cascade_wrapper(self):
        raise NotImplementedError(
            "Maca FlashInfer backend does not support cascade attention."
        )
    ...
    def build(
        ...
        # Metax does not support cascade attention for now
        use_cascade = False and common_prefix_len > 0
        if use_cascade:
...
class FlashInferImpl(AttentionImpl):
    ...
    def forward(
        ...
        stride_order = MacaFlashInferBackend.get_kv_cache_stride_order()
        ...
        if num_prefill_tokens > 0:
            ...
            if not attn_metadata.prefill_use_trtllm:
                ...
                raise NotImplementedError(
                    "prefill_use_trtllm prefill attention is not implemented on MACA yet."
                )
        ...
        if num_decode_tokens > 0:
            ...
            if not attn_metadata.decode_use_trtllm:
                ...
                raise NotImplementedError(
                    "prefill_use_trtllm prefill attention is not implemented on MACAyet."
                )
