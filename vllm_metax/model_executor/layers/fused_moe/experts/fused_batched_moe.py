...
def moe_mmk(
    ...
    accumulator = tl.zeros(
        (BLOCK_M, BLOCK_N), dtype=tl.int32 if use_int8_w8a8 else tl.float32
    )
    ...
    if use_w8a16:
        ...
        if group_k > 0 and group_n > 0:
            ...
            accumulator = (accumulator.to(tl.float32) * a_scale * b_scale).to(
                compute_type
            )
    else:
        accumulator = accumulator.to(compute_type)
...
class BatchedTritonExperts(mk.FusedMoEExpertsModular):
    ...
    def apply(
        ...
        elif hidden_states.dtype in [current_platform.fp8_dtype(), torch.int8]:
            compute_type = tl.bfloat16
        else:
