...
def fused_moe_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K * SPLIT_K)):
        ...
        # K dimension.
        if USE_TD:
            a = a_desc.gather(gather_idx, k * BLOCK_SIZE_K * SPLIT_K)
            b = b_desc.load([pid_n * BLOCK_SIZE_N, k * BLOCK_SIZE_K * SPLIT_K]).T
        elif SWAP_AB:
            a_mask = (offs_k[:, None] < K - k * BLOCK_SIZE_K * SPLIT_K) & token_mask[
                None, :
            ]
            b_mask = offs_k[None, :] < K - k * BLOCK_SIZE_K * SPLIT_K
...
def invoke_fused_moe_wna16_triton_kernel(
    ...
    # Triton kernel "fused_moe_kernel_gptq_awq" has been maintained by mcoplib
    # since v0.18.0-dev .
    if mx_envs.USE_PRECOMPILED_KERNEL:
        fused_moe_triton_kernel_gptq_awq(
            grid,
...
def invoke_fused_moe_triton_kernel(
    ...
    if use_int4_w4a8 and not mx_envs.MACA_VLLM_ENABLE_MCTLASS_FUSED_MOE:
        raise NotImplementedError(
            "W4A8 MoE requires MACA_VLLM_ENABLE_MCTLASS_FUSED_MOE=1; "
            "the Triton fallback does not support W4A8 packed weights."
        )
    ...
    elif use_int4_w4a8 and mx_envs.MACA_VLLM_ENABLE_MCTLASS_FUSED_MOE:
...
def get_moe_configs(
    ...
    # --------------------------------------------------------
    # Metax Note: Metax defined additional parameter:
    #       - H dimension
    ...
    #
    # The priority order for searching config in maca is:
    #   1. user_defined_config_folder of `configs/H={hidden_size}/`
    #   2. default config folder `configs/`
    # --------------------------------------------------------
    user_defined_config_folder = envs.VLLM_TUNED_CONFIG_FOLDER
    logger.info_once("User defined config folder: %s", user_defined_config_folder)
...
def get_default_config(
    ...
    # num_stages can cause triton.runtime.errors.OutOfResources on Maca.
    num_stages_maca = 2
...
def _prepare_expert_assignment(
    ...
    *,
    ...
    # Same as upstreams _prepare_expert_assignment

    # MCTLASS uses aligned assignments; the naive path is Triton-only.
...
def fused_experts_impl(
    ...
    # So we use our own version of `get_config_dtype_str`.
    # ---------------------------------------------------------------
    ...
    # ┌------------------------  Metax Modification -------------------------┐
    staged_configs = initialize_staged_config(config)
    # └------------------------- Metax Modification -------------------------┘
    ...
    if mx_envs.MACA_VLLM_ENABLE_MCTLASS_FUSED_MOE:
        maybe_override_stage_block_size_m(
            ...
            use_int4_w4a8=use_int4_w4a8,
            use_int4_w4a16=use_int4_w4a16,
            quantized_hidden_states=qhidden_states,
        )
    ...
    # Only MCTLASS consumes this flag. Alignment above drops invalid experts.
    # _prepare_expert_assignment set ignore_invalid_experts to True here,
    # filter_expert is False.
    filter_expert = not mx_envs.MACA_VLLM_ENABLE_MCTLASS_FUSED_MOE
    ...
    if staged_configs[0]["BLOCK_SIZE_M"] != staged_configs[1]["BLOCK_SIZE_M"]:
