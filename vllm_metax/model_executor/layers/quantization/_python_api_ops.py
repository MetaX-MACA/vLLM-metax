...
def mctlassEx_fused_moe_bf16_get_kernel_m(A: torch.Tensor,
                                          B: torch.Tensor,
                                          C: torch.Tensor,
                                          num_experts: int,
                                          batch_size: int,
    ...
    kernel_m = mctlass_moe_gemm.get_kernel_m(A, B, C, num_experts, batch_size, N, K, topk)
    return kernel_m
...
def mctlassEx_fused_moe_bf16_gemm(
    ...
    assert mctlass_moe_gemm is not None, "mctlass op is not imported correctly"
    mctlass_moe_gemm(
        batch_size, N, K, num_experts, EM, topk,
        A, B, C, scale_a, scale_b, bias,
        topk_weights, token_ids, expert_ids, num_tokens_post_padded,
        mul_routed_weight)
    return C
