...
def run(command):
    ...
    try:
        ...
        if command == "mx-smi topo -m":
            # don't remove the leading whitespace of `mx-smi topo -m`
...
def get_bios_version(run_lambda):
    return run_and_parse_first_match(run_lambda, "mx-smi", r"BIOS Version:\s*([^\s]+)")
...
def get_nvidia_smi():
    # Note: mx-smi is currently available only on Windows and Linux
    smi = "mx-smi"
    if get_platform() == "win32":
        system_root = os.environ.get("SYSTEMROOT", "C:\\Windows")
