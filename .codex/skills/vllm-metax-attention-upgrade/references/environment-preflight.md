...
```bash
/opt/venv/bin/python .codex/skills/vllm-metax-attention-upgrade/scripts/probe_environment.py \
  --vllm-source /workspace/vllm \
  --metax-source /workspace/vLLM-metax \
  --runtime-cwd /workspace/vLLM-metax \
  > /tmp/metax-attention-environment.json
```
...
| Components | All matching DeepGEMM, FlashAttention, FlashMLA, MCOPLIB, Torch and Triton records, plus selected top-level module origins. |
...
`inspect.signature(flash_attn.flash_attn_varlen_func)` and
`inspect.getsourcefile(deep_gemm.bf16_mqa_logits)`. Follow wrapper aliases to the selected
