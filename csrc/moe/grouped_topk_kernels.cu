...
__forceinline__ __device__ bool is_better_than(T val, T baseline) {
  ...
  float f_baseline = static_cast<float>(baseline);
  return (f_val > f_baseline && greater) || (f_val < f_baseline && !greater);
}
...
__forceinline__ __device__ bool is_better_than(T val, T baseline, idxT index,
  ...
  float f_baseline = static_cast<float>(baseline);
  bool res =
      (f_val > f_baseline && greater) || (f_val < f_baseline && !greater);
  if (f_val == f_baseline) {
  ...
  __device__ static void sort(T* __restrict__ val_arr,
    ...
    for (int stage = 0; stage < 4; ++stage) {
      for (int stride = (1 << stage); stride > 0; stride /= 2) {
        ...
        bool is_better;
        float f_val = static_cast<float>(*val_arr);
        float f_other = static_cast<float>(other);

        if constexpr (is_stable) {
          if constexpr (ascending) {
            is_better = ((f_val > f_other) ||
                         ((f_val == f_other) && (*idx_arr < other_idx))) !=
                        (reverse != is_second);
          } else {
            is_better = ((f_val > f_other) ||
                         ((f_val == f_other) && (*idx_arr > other_idx))) !=
          ...
          is_better =
              (f_val != f_other && (f_val > f_other) != (reverse != is_second));
        }
        if (is_better) {
          *val_arr = other;
          *idx_arr = other_idx;
  ...
  __device__ static void merge(T* __restrict__ val_arr,
    ...
    for (int stride = WARP_SIZE / 2; stride > 0; stride /= 2) {
      ...
      if constexpr (is_stable) {
        ...
        // 注意：原代码的 val 其实就是 *val_arr 的引用，所以用 f_val
        // 替代是安全的
        is_better = (f_val != f_other &&
                     ((f_val > f_other) == (ascending != is_second)));
      }
...
__device__ void topk_with_k2(T* output, T const* input, BiasT const* bias,
  ...
  if (num_experts_per_group > WARP_SIZE) {
    for (int i = lane_id; i < num_experts_per_group; i += WARP_SIZE) {
      ...
      if (static_cast<float>(value) > static_cast<float>(largest)) {
        second_largest = largest;
        largest = value;
      } else if (static_cast<float>(value) >
  ...
  // float max1_f = cg::reduce(tile, static_cast<float>(largest),
  // cg::greater<float>());
  T max1 = static_cast<T>(
      cg::reduce(tile, static_cast<float>(largest), cg::greater<float>()));

  T max2 = max1;
  bool equal_to_max1 =
      (static_cast<float>(max1) == static_cast<float>(largest));

  int count_max1 = __popc(__ballot_sync(FULL_WARP_MASK, equal_to_max1));

  if (count_max1 == 1) {
    largest = (static_cast<float>(largest) == static_cast<float>(max1))
                  ? second_largest
                  : largest;
    max2 = static_cast<T>(
        cg::reduce(tile, static_cast<float>(largest), cg::greater<float>()));
    // max2 = static_cast<T>(max2_f);
  }

  if (lane_id == 0) {
    *output =
        static_cast<T>(static_cast<float>(max1) + static_cast<float>(max2));
  }
}
...
__global__ void grouped_topk_fused_kernel(
  ...
  if (topk_group_i32 > 0) {
    ...
    proceed = (static_cast<float>(kth_val) != static_cast<float>(neg_inf<T>()));
  }
    ...
    for (int32_t i = lane_id; i < align_num_experts_per_group; i += WARP_SIZE) {
      ...
      if (i < num_experts_per_group) {
        ...
        if (is_finite(input)) {
          ...
          cand = static_cast<T>(static_cast<float>(score) +
                                static_cast<float>(bias[idx]));
...
void invokeNoAuxTc(T* scores, float* topk_values, IdxT* topk_indices,
  ...
  if (is_single_group || is_multi_group) {
    ...
    // MACA 平台直接降级为标准启动语法，绕开不支持的属性
    kernel_instance<<<config.gridDim, config.blockDim, config.dynamicSmemBytes,
                      config.stream>>>(scores, topk_values, topk_indices, bias,
                                       num_tokens, n_group, topk_group, topk,
                                       num_experts, num_experts / n_group,
