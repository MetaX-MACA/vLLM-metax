...
void fused_deepseek_v4_qnorm_rope_kv_rope_insert(
    ...
    torch::stable::Tensor& k_cache,  // [num_blocks, block_stride_elements] bf16
    torch::stable::Tensor const& slot_mapping,   // [N] int64
    torch::stable::Tensor const& position_ids,   // [N] int64
    torch::stable::Tensor const& cos_sin_cache,  // [max_pos, rope_dim] fp32
  ...
  STD_TORCH_CHECK(
      position_ids.device().is_cuda() &&
          position_ids.scalar_type() == torch::headeronly::ScalarType::Long,
      "position_ids must be int64 CUDA");
  STD_TORCH_CHECK(cos_sin_cache.device().is_cuda(),
                  "cos_sin_cache must be CUDA");
  ...
  STD_TORCH_CHECK(k_cache.scalar_type() == q.scalar_type(),
                  "k_cache dtype must match q/kv (bfloat16)");
  STD_TORCH_CHECK(
      cos_sin_cache.scalar_type() == torch::headeronly::ScalarType::Float,
      "cos_sin_cache must be float32");
  STD_TORCH_CHECK(k_cache.dim() == 2,
  ...
  VLLM_STABLE_DISPATCH_HALF_TYPES(
      q.scalar_type(), "fused_deepseek_v4_qnorm_rope_kv_insert", [&] {
        ...
        vllm::deepseek_v4_fused_ops::launchFusedDeepseekV4QNormRopeKVRopeInsert<
            ...
            reinterpret_cast<qkv_scalar_t*>(k_cache.mutable_data_ptr()),
            reinterpret_cast<int64_t const*>(slot_mapping.const_data_ptr()),
            reinterpret_cast<int64_t const*>(position_ids.const_data_ptr()),
