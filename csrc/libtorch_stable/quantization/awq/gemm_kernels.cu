...
#include "libtorch_stable/quantization/gptq/hgemm_gptq.h"
...
torch::stable::Tensor awq_to_gptq_4bit(torch::stable::Tensor qweight) {
  const torch::stable::accelerator::DeviceGuard device_guard(
      qweight.get_device_index());
  const cudaStream_t stream = get_current_cuda_stream();

  const uint32_t* qweight_ptr =
      reinterpret_cast<const uint32_t*>(qweight.mutable_data_ptr<int>());
...
torch::stable::Tensor awq_dequantize(torch::stable::Tensor _kernel,
  ...
  if (_scaling_factors.scalar_type() == torch::headeronly::ScalarType::Half) {
    ...
    auto scaling_factors = reinterpret_cast<maca_bfloat16*>(
        _scaling_factors.mutable_data_ptr<torch::headeronly::BFloat16>());
...
torch::stable::Tensor awq_gemm(
  ...
  auto zeros = reinterpret_cast<int*>(_zeros.mutable_data_ptr<int>());
  auto temp_space =
      reinterpret_cast<float*>(_temp_space.mutable_data_ptr<float>());
