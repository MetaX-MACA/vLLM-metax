...
void cutlass_moe_mm_sm75(torch::Tensor& out, torch::Tensor const& a,
  ...
  using mctlassMoeGemmBf16Op =
      mctlassMoeGemm<ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC,
                     ElementCompute, ArchTag>;
  ...
  typename mctlassMoeGemmBf16Op::Arguments arguments{
      ...
      b_ptr,
      c_ptr,
      {token_ids_ptr, expert_ids_ptr, num_tokens_post_padded_ptr,
       num_tokens_post_padded_size, static_cast<int32_t>(topk),
       mul_routed_weight}};
...
int64_t cutlass_moe_mm_gemm_kernel_m_w8a8_sm75(int64_t num_valid_tokens,
  ...
  mctlass::gemm::GemmCoord kernel_size;
  mctlass::Status status =
      mctlass_op.gemm_kernel_mnk(problem_size, kernel_size);
  if (status != mctlass::Status::kSuccess) {
    return -1;
  }
...
void cutlass_moe_mm_w8a8_sm75(torch::Tensor const& a, torch::Tensor const& b,
                              ...
                              torch::Tensor const& expert_ids,
                              torch::Tensor const& num_tokens_post_padded,
                              int64_t N, int64_t K, int64_t EM,
                              int64_t num_valid_tokens, int64_t topk,
                              bool mul_routed_weight) {
  using ElementA = int8_t;
  using ElementB = int8_t;
  ...
  typename mctlassMoeGemmOp::Arguments arguments{
      ...
      c_ptr,
      {token_ids_ptr, expert_ids_ptr, num_tokens_post_padded_ptr,
       static_cast<int32_t>(EM), static_cast<int32_t>(topk),
       mul_routed_weight}};
