...
__device__ __forceinline__ void dequant_fma_awq_int4(
    const quant_packed_type& a, const v2f (&scale)[m_per_thread / 2],
    const v2f (&zero)[m_per_thread / 2], const v2f (&b)[N],
...
__global__ __launch_bounds__(256) void hgemv_nn_splitk_awq(
  ...
  int this_group_elements = (blockIdx.x + 1) * group_elements <= m
                                ? group_elements
                                : m - blockIdx.x * group_elements;
  ...
  for (int i = 0; i < k; i += LoopNum * thread_groups) {
    ...
    for (int x = tid; x < loading_count; x += ThreadBlock) {
      ...
      //*((float*)temp_scales) = ((float*)(scales + quant_group * scales_stride
      //+ m_offset_scales))[x];
      temp_scales[0] =
          scales[quant_group * scales_stride + m_offset_scales + dest_offset_0];
      temp_scales[1] =
          scales[quant_group * scales_stride + m_offset_scales + dest_offset_1];
    ...
    if (m_index < this_group_elements) {
      ...
      for (int c = 0; c < m_per_thread / 2; c++)
        local_zeros[c] = {bsm_zeros_ptr[m_index + c * 2],
                          bsm_zeros_ptr[m_index + c * 2 + 1]};

#define DEQUANT_FMA(a, b)                                                \
  dequant_fma_awq_int4<N, m_per_thread>(a, local_scales, local_zeros, b, \
                                        c_splited);

      quant_packed_type A[4];
      const int packed_a_stride = srcAStride / PACK_RATIO;
      int src_a_offset =
          (loop_index + i + tidCol * LoopNum) * srcAStride / PACK_RATIO +
          m_offset_a + m_per_thread * tidRow / PACK_RATIO;
      ...
      for (; loop_index < LoopNum - 4; loop_index += 4) {
        ...
        for (int y = 0; y < N; y++) {
          float s[4];
          *(float4*)s = *(float4*)(bsm_b_ptr + tidCol * LoopNum + loop_index +
  ...
  for (int y = 0; y < N; y++) {
    if (m_index < this_group_elements) {
      for (int i = 0; i < m_per_thread / 2; i++) {
        smem[tidCol + (tidRow * m_per_thread + i * 2) * ThreadBlock /
                          BlockDimX] = c_splited[y][i].x;
        smem[tidCol + (tidRow * m_per_thread + i * 2 + 1) * ThreadBlock /
                          BlockDimX] = c_splited[y][i].y;
      }
    }
    __syncthreads();
...
__global__ __launch_bounds__(256) void hgemv_nn_splitk_awq_kb128(
    const half* __restrict__ srcB, const quant_packed_type* __restrict__ srcA,
    const quant_packed_type* __restrict__ zeros,
    const half* __restrict__ scales, half* dst, int m, int n, int k,
    int srcAStride, int dstStride, int k_div_sk,
  ...
  if (b_perm != nullptr) {
    for (int y = 0; y < N; y++) {
      for (int x = tidRow; x < LoopNum; x += BlockDimX) {
        bsm_b_ptr[x + tidCol * LoopNum + y * LoopNum * thread_groups] =
            srcB[b_perm[splitKOffset + tidCol * LoopNum + x] + y * k_stride];
      }
    }
  } else {
    for (int y = 0; y < N; y++) {
      for (int x = tidRow; x < LoopNum; x += BlockDimX) {
        bsm_b_ptr[x + tidCol * LoopNum + y * LoopNum * thread_groups] =
            srcB[splitKOffset + tidCol * LoopNum + x + y * k_stride];
      }
    }
  }
...
#define DEQUANT_FMA(a, b)                                                \
  dequant_fma_awq_int4<N, m_per_thread>(a, local_scales, local_zeros, b, \
    ...
    const int packed_a_stride = srcAStride / PACK_RATIO;
    int src_a_offset =
        (loop_index + tidCol * LoopNum) * srcAStride / PACK_RATIO + m_offset_a +
        m_per_thread * tidRow / PACK_RATIO;
    A[0] = srcA[src_a_offset];
    src_a_offset += packed_a_stride;
    A[1] = srcA[src_a_offset];
