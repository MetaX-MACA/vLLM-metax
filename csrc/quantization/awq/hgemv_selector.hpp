...
struct KernelEventRecorder {
  ...
  void start() { mcEventRecord(_start, NULL); }
...
struct GemvParamAutoSelector {
 ...
 public:
  GemvParamAutoSelector(int _m, int _n, int _k,
                        bool allow_imcomplete_bx = false)
      : m(_m), k(_k) {
    if (!prepare_parameters(_m, _n, _k, block_x_range.second,
                            split_k_range.second, allow_imcomplete_bx))
      return;
    block_x_range.first = block_x_range.second;
    split_k_range.first = split_k_range.second;
    for (int i = 0; i < 3; i++)
      if (block_x_range.first / 2 >= 16) block_x_range.first /= 2;
    for (int i = 0; i < 3; i++)
      if (split_k_range.first % 2 == 0 && split_k_range.first / 2 > 0)
        split_k_range.first /= 2;
    ...
    data_size_in_bytes +=
        (uint64_t)_m * _k / quant_group / pack_ratio * sizeof(uint32_t);
  ...
  void select_in_warmup(const std::function<bool(int, int)>& f) {
    ...
    // Warmup
    for (auto iter = param_candidates.begin(); iter != param_candidates.end();
         iter++) {
      for (int i = 0; i < 5; i++) {
        auto& p = *iter;
        f(p.first, p.second);
  ...
  void run(const std::function<bool(int, int)>& f) {
    ...
    if (warmup_iters > 0) {
      ...
      if (current_block_x > block_x_range.first)
        current_block_x /= 2;
      else if (current_split_k > split_k_range.first)
        current_split_k /= 2;
      if (current_block_x == block_x_range.first &&
          current_split_k >= split_k_range.first) {
        current_block_x = block_x_range.second;
        current_split_k = split_k_range.second;
      }
      warmup_iters--;
    ...
    if (_selected) {
      ...
      // current_split_k, current_perf_iter, ms);
      if (!launched) {
      ...
      if (current_split_k > split_k_range.first) {
        ...
        printf(
            "Error: all parameters are tried! current_block_x = %d, "
            "current_split_k = %d, block_x_range = %d,%d, split_k_range = "
            "%d,%d, best_block_x = %d, best_split_k = %d\n",
            current_block_x, current_split_k, block_x_range.first,
            block_x_range.second, split_k_range.first, split_k_range.second,
  ...
  float gemv_ave_time_us_cost() { return kernel_best_time_ms_ave; }

  float gemv_bandwidth() {
    return data_size_gb / (kernel_best_time_ms_ave / 1000.0f);
  }

  static bool prepare_parameters(int m, int n, int k, int& bx, int& sk,
                                 bool allow_imcomplete_bx = false) {
    if (n > 4) return false;
    ...
    int max_split_k = k / quant_group;
    int proper_splitk = 1;
    while (proper_splitk * 2 <= 256 && proper_splitk * 2 <= max_split_k)
      proper_splitk *= 2;

    int thread_block = 256;  // A thread block of 512 will wait for more warps
                             // when doing syncthreads

    int proper_bx = 16;
    if (m % (proper_bx * m_per_thread) != 0) return false;
    // proper_bx * m_per_threads corresponds to group_elements, this value
    // should not be larger than 2048 as limit of shared memory
    while (proper_bx * 2 * m_per_thread &&
           (m % (proper_bx * 2 * m_per_thread) == 0) && proper_bx * 2 <= 256 &&
           proper_bx * 2 * m_per_thread <= 2048)
      proper_bx *= 2;
    if (allow_imcomplete_bx) {
      int may_proper_bx = proper_bx * 2;
      if (may_proper_bx <= 256 && may_proper_bx * m_per_thread <= 2048 &&
          (m % (may_proper_bx * m_per_thread)) >
              may_proper_bx * m_per_thread / 4) {
...
class GemvSelectorHolder {
 private:
  std::vector<GemvParamAutoSelector<quant_group, pack_ratio, m_per_thread>>
      _selectors;
  static std::shared_ptr<
      GemvSelectorHolder<quant_group, pack_ratio, m_per_thread>>
      _holder;
  static GemvParamAutoSelector<quant_group, pack_ratio, m_per_thread>
      _invalid_selector;

 public:
  static GemvParamAutoSelector<quant_group, pack_ratio, m_per_thread>& selector(
      int m, int n, int k, bool allow_imcomplete_bx = false) {
    if (!GemvSelectorHolder::_holder) {
      GemvSelectorHolder::_holder.reset(
          new GemvSelectorHolder<quant_group, pack_ratio, m_per_thread>());
    }
    int bx, sk;
    if (!GemvParamAutoSelector<quant_group, pack_ratio, m_per_thread>::
            prepare_parameters(m, n, k, bx, sk, allow_imcomplete_bx)) {
      return _invalid_selector;
    }
    auto iter = std::find_if(
        _holder->_selectors.begin(), _holder->_selectors.end(),
        [&](const GemvParamAutoSelector<quant_group, pack_ratio, m_per_thread>&
                p) -> bool { return p.m == m && p.k == k; });
    if (iter != _holder->_selectors.end()) return *iter;
    GemvParamAutoSelector<quant_group, pack_ratio, m_per_thread> sl(
        m, n, k, allow_imcomplete_bx);
    if (!sl.valid()) {
      return _invalid_selector;
    }
...
std::shared_ptr<GemvSelectorHolder<quant_group, pack_ratio, m_per_thread>>
    GemvSelectorHolder<quant_group, pack_ratio, m_per_thread>::_holder;

template <int quant_group, int pack_ratio, int m_per_thread>
GemvParamAutoSelector<quant_group, pack_ratio, m_per_thread> GemvSelectorHolder<
    quant_group, pack_ratio, m_per_thread>::_invalid_selector(0, 8, 0);
}  // namespace hgemv_selector