...
We will make a block size 256, which means 4 waves, we need all these 4 waves
execute on one PEU So the maximum shared memory used by a block use not larger
than 16K, and MTRegisters should be less than 128.

A tile will be divied into serveral sub_tiles, a minimum sub_tile is m16n16k32,
so we can have m_blocks*k_blocks when loading a and k_blocks*n_blocks when
loading b.

We will have n*k/TILE_N/TILE_K tiles in b, and these TILES are diveded into
ITERS where ITERS*PEUS>=TILES for a proper shape. Of course some small shapes
will nerver have chance to use all the PEUS

Parallism is not suitable for C500 as parallism will dequant b more than once,
...
#define LOADING_A_LOOP \
  SLICE_K* TILE_M / (sizeof(PackType) / sizeof(scalar_t)) / THREADS
...
template <const int THREADS,  // number of threads in a threadblock
          const int PACK_NUM, const bool USE_C = false>
__global__ void all_reduce(float* in_data, maca_bfloat16* out_data,
                           size_t num_elem) {
  int bidx = blockIdx.x;
  int tidx = threadIdx.x;
  int idx = (bidx * THREADS + tidx) * PACK_NUM;
  ...
  if constexpr (PACK_NUM == 4) {
    if constexpr (USE_C == true) {
      ...
      ldg_b64_reg_noasync(*((b64VecType*)temp_out_bf16),
                          ((b64VecType*)(out_data + idx)), pred, true);
      ldg_b128_reg_noasync(*((b128VecType*)temp_in_fp32),
                           ((b128VecType*)(in_data + idx)), pred, true);
...
constexpr static int PEUS = 13 * 8 * 4;  // For C500, There are 8 DPC and each
                                         // DPC have 13 APs, each AP have 4 PEUs
...
// #define CVT_B0TOF32(q, out) asm volatile("cvt_b0tof32
// %0,%1;\n":"=r"(out):"r"(q)); #define CVT_B1TOF32(q, out) asm
// volatile("cvt_b1tof32 %0,%1;\n":"=r"(out):"r"(q)); #define CVT_B2TOF32(q,
// out) asm volatile("cvt_b2tof32 %0,%1;\n":"=r"(out):"r"(q)); #define
// CVT_B3TOF32(q, out) asm volatile("cvt_b3tof32 %0,%1;\n":"=r"(out):"r"(q));
...
struct TileManager {
  ...
  __device__ __forceinline__ void init(int m, int n, int k, int bidx,
                                       int iters) {
    // Calculate tile start row and cols so we can calculate the offset address
    // of a b and c
    ...
    my_iters = tile_idx + iters >= tiles_n * tiles_k
                   ? tiles_n * tiles_k - tile_idx
                   : iters;
  ...
  // The point is when all quanted values are dequanted and all a are stored to
  // bsm already Then the registers for scales, zeros, and a_caches are free to
  // use, bsm for scales are already free to use. The main problem we will face
  // is that no more registers can be used to cache tile information when we
  // want to preload next tile data
...
So every TILE we load 64x64x2=8192bytes A, 128x64/8x4=4096bytes B, 128x4=512
scales, 128x4=512 zeros if needed

If more batches is needed, I think it is better to run dequant again. We do not
count on situation that m>128 now.

Memory View:
There are 1/2/4 waves according to BLOCKS_N, wave_count = BLOCKS_N, mostly,
BLOCKS_N=4, and THREADS=256 Each wave will process a m16n16k32 sub tile with two
mma instructions, we should re-order data in bsm so we can load data from shared
memory only with tid or wave_tid, where we can reduce the instructions of
calculate address offsets of each data:

BSM A order(assume that BLOCKS_M=4):
|---------------------------------8x4x16 half value
------------------------------------------------------------------------|
|-use by wave  slotidx 0 slottid 0------|-use by wave  slotidx 0 slottid
1------|...|-use by wave  slotidx 0 slottid 15-----|
|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8
half--|...|-8 half--|-8 half--|-8 half--|-8 half--|
|m00k00~07|m16k00~07|m32k00~07|m48k00~07|m01k00~07|m17k00~07|m33k00~07|m49k00~07|...|m15k00~07|m31k00~07|m47k00~07|m63k00~07|

|-use by wave  slotidx 1 slottid 0------|-use by wave  slotidx 1 slottid
1------|...|-use by wave  slotidx 1 slottid 15-----|
|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8
half--|...|-8 half--|-8 half--|-8 half--|-8 half--|
...
|-use by wave  slotidx 3 slottid 0------|-use by wave  slotidx 3 slottid
1------|...|-use by wave  slotidx 3 slottid 15-----|
|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8 half--|-8
half--|...|-8 half--|-8 half--|-8 half--|-8 half--|
...
When loading a from bsm, each thread only read 4x8 half value with offset
wave_tid: b128 out[4]; b128 *bsm_a_ptr; out[0] = bsm_a_ptr[wave_tid*BLOCKS_M];
out[1] = bsm_a_ptr[wave_tid*BLOCKS_M+1];
out[2] = bsm_a_ptr[wave_tid*BLOCKS_M+2];
out[3] = bsm_a_ptr[wave_tid*BLOCKS_M+3];


BSM B order: here k means the quanted k, which means each k represents 8
dequanted k data
...
*/
template <class scalar_t>
__device__ __forceinline__ void awq_dequant_4bits(const uint32_t& p,
                                                  scalar_t (&out)[8],
                                                  scalar_t (&scale)[8],
                                                  const uint32_t& scale_zero) {
  if constexpr (std::is_same_v<scalar_t, __maca_bfloat16>) {
    ...
    *((v2f*)tmp) = __builtin_mxc_pk_fma_f32(v2zero, v2neg, *((v2f*)tmp));
    v2scale[0] = (float)scale[0];
    v2scale[1] = (float)scale[1];
    ...
    *((v2f*)tmp) = __builtin_mxc_pk_fma_f32(v2zero, v2neg, *((v2f*)tmp));
    v2scale[0] = (float)scale[4];
    v2scale[1] = (float)scale[5];
    *((v2f*)tmp) = __builtin_mxc_pk_fma_f32(*((v2f*)tmp), v2scale, v2z);
    ...
    CVT_B2TOF32(z0, v2zero[1]);
    *((v2f*)tmp) = __builtin_mxc_pk_fma_f32(v2zero, v2neg, *((v2f*)tmp));
    v2scale[0] = (float)scale[2];
    v2scale[1] = (float)scale[3];
    *((v2f*)tmp) = __builtin_mxc_pk_fma_f32(*((v2f*)tmp), v2scale, v2z);
    ...
    *((v2f*)tmp) = __builtin_mxc_pk_fma_f32(v2zero, v2neg, *((v2f*)tmp));
    v2scale[0] = (float)scale[6];
    v2scale[1] = (float)scale[7];
    *((v2f*)tmp) = __builtin_mxc_pk_fma_f32(*((v2f*)tmp), v2scale, v2z);
...
template <class scalar_t>
__device__ __forceinline__ void dequant_gptq_8bits(const PackType& p,
                                                   scalar_t (&out)[4],
                                                   const v2f& scale,
                                                   const v2f& scale_zero) {
  v2f a0;
...
// decompress zero
__device__ __forceinline__ void decompress_zero_4bits(const PackType& zp,
                                                      float (&out)[8]) {
  v2f a0;
...
template <typename scalar_t, const vllm::ScalarTypeId w_type_id, int THREADS,
          int BLOCKS_M, int BLOCKS_N, int BLOCKS_K, bool HAS_ACT, bool HAS_ZP,
          bool HAS_M_PRED, bool HAS_NK_PRED>
struct LoadingManager {
  ...
  // 2. C500 has no atomic instructions for bfloat16, we cannot atomic a
  // bfloat16 memory
  // 3. The perfect precision type of atomic should be fp32, but the cost is too
  // high to allocate a temp memory for float atomic
  ...
  __device__ __forceinline__ void set_address(
      const PackTypeInt4* a, const PackTypeInt4* b, PackTypeInt4* c,
      PackTypeInt4* c_temp, const PackTypeInt4* scale_ptr,
  ...
  __device__ __forceinline__ void next_k() {
    ...
    bsm_a_ptr += tv.slot_tid *
                     (PAD_SLICE_K / (sizeof(PackTypeInt4) / sizeof(scalar_t))) +
                 tv.slot_idx;
  }
  ...
  __device__ __forceinline__ void next_k_pre() {
    ...
    if constexpr (w_type_id == vllm::kU4.id() ||
                  w_type_id == vllm::kU4B8.id()) {
      B_loading += SLICE_K / PACK_RATIO_4BITS * n;
    } else if constexpr (w_type_id == vllm::kU8.id() ||
                         w_type_id == vllm::kU8B128.id()) {
...
#pragma unroll LOADING_A_LOOP
    for (int i = 0; i < LOADING_A_LOOP; i++) {
      ...
      // FIXME: we cannot do slice k pad as ldg_b32_bsm_async seems does not
      // support padding
      if constexpr (HAS_M_PRED && HAS_NK_PRED) {
        ...
        bool pred_k =
            k_broad + reading_k * FragACount < k && tile_manager.global_pred;
        ldg_b32_reg_noasync(temp_a[i], gvm_addr, pred_k, true);
      } else {
        ldg_b32_reg_noasync(temp_a[i], gvm_addr, tile_manager.global_pred,
                            true);
      }
      t += THREADS;
    }
...
#pragma unroll LOADING_A_LOOP
  ...
  __device__ __forceinline__ void lds_a(int midx) {
    *((PackTypeInt4*)local_a[midx]) = *bsm_a_ptr;
    bsm_a_ptr +=
        SLOT * (PAD_SLICE_K / (sizeof(PackTypeInt4) / sizeof(scalar_t)));
  }
  ...
  __device__ __forceinline__ void ldg_b(int k_idx, int korder = 0) {
    if constexpr (HAS_NK_PRED) {
      bool pred_k = tile_manager.tile_start_row * TILE_K + k_idx * SLICE_K +
                        tv.slot_idx * PACK_RATIO_4BITS + korder <
                    k;
      bool pred_n = tile_manager.tile_start_col * TILE_N +
                        (tv.wave_idx * SLOT + tv.slot_tid) * N_ITERS <
                    n;
      bool pred = pred_n && pred_k && tile_manager.global_pred;
      FragB* addr = (FragB*)B_loading + korder * n;
      if constexpr (N_ITERS == 2) {
        ldg_b64_reg_noasync(*((PackTypeInt2*)local_b_cache),
                            ((PackTypeInt2*)addr), pred, true);
      } else if constexpr (N_ITERS == 4) {
        ldg_b128_reg_noasync(*((PackTypeInt4*)local_b_cache),
                             ((PackTypeInt4*)addr), pred, true);
      }
    } else {
      FragB* addr = (FragB*)B_loading + korder * n;
      if constexpr (N_ITERS == 2) {
        ldg_b64_reg_noasync(*((PackTypeInt2*)local_b_cache),
                            ((PackTypeInt2*)addr), tile_manager.global_pred,
                            true);
      } else if constexpr (N_ITERS == 4) {
        ldg_b128_reg_noasync(*((PackTypeInt4*)local_b_cache),
                             ((PackTypeInt4*)addr), tile_manager.global_pred,
                             true);
      }
    }
  }
  ...
  __device__ __forceinline__ void ldg_scales() {
    bool pred =
        tv.tid < TILE_N / (sizeof(FragScaleLoading) / sizeof(scalar_t)) &&
        tile_manager.global_pred;
    if constexpr (HAS_NK_PRED) {
      pred = pred && tv.tid < (n - tile_manager.tile_start_col * TILE_N) /
                                  (sizeof(FragScaleLoading) / sizeof(scalar_t));
    }
    // FragScaleLoading *scale_bsm = (FragScaleLoading*)(smem_base + 0x2000) +
    // tv.tid;
  ...
  __device__ __forceinline__ void ldg_zp() {
    if constexpr (w_type_id == vllm::kU4.id()) {
      bool pred =
          (tv.tid < TILE_N / PACK_RATIO_4BITS) && tile_manager.global_pred;
      if constexpr (HAS_NK_PRED) {
        pred = pred && tv.tid < ((n - tile_manager.tile_start_col * TILE_N) /
                                 PACK_RATIO_4BITS);
      }
  ...
  __device__ __forceinline__ void sts_scales() {
    FragScaleLoading* scale_bsm =
        (FragScaleLoading*)(smem_base + 0x2000) + tv.tid;
  ...
  __device__ __forceinline__ void sts_zeros() {
    if constexpr (w_type_id == vllm::kU4.id()) {
      ...
      if (pred) {
        ...
        float* scale_bsm =
            (float*)(smem_base + 0x3000) + tv.tid * PACK_RATIO_4BITS;
...
#pragma unroll
  ...
  __device__ __forceinline__ void dequant(int kdx, int korder = 0) {
    if constexpr (w_type_id == vllm::kU4.id() ||
                  w_type_id == vllm::kU4B8.id()) {
      dequant_gptq_4bits<scalar_t>(local_b[kdx], local_dequanted_b[kdx],
                                   local_scales[kdx], local_zeros[kdx]);
    } else if constexpr (w_type_id == vllm::kU8.id() ||
                         w_type_id == vllm::kU8B128.id()) {
      dequant_gptq_8bits<scalar_t>(local_b[kdx],
                                   local_dequanted_b_8bits[kdx][korder],
                                   local_scales[kdx], local_zeros[kdx]);
    }
  }
...
#pragma unroll
      for (int i = 0; i < N_ITERS; i++) {
        ...
        mma_16x16x16<scalar_t>(local_a[mdx][1],
                               *((PackTypeInt2*)local_dequanted_b[i] + 1),
                               *((PackTypeInt4*)output[mdx][i]));
      }
    } else if constexpr (w_type_id == vllm::kU8.id() ||
                         w_type_id == vllm::kU8B128.id()) {
#pragma unroll
      for (int i = 0; i < N_ITERS; i++) {
        mma_16x16x16<scalar_t>(local_a[mdx][0],
                               *((PackTypeInt2*)local_dequanted_b_8bits[i][0]),
                               *((PackTypeInt4*)output[mdx][i]));
        mma_16x16x16<scalar_t>(local_a[mdx][1],
                               *((PackTypeInt2*)local_dequanted_b_8bits[i][1]),
                               *((PackTypeInt4*)output[mdx][i]));
      }
    }
  }
...
#pragma unroll
  ...
  __device__ __forceinline__ void init_address_pre(int _m, int _n, int _k,
                                                   int _quant_group_power2,
                                                   int _bidx, int _iters,
                                                   uint8_t* _smem_base) {
    tv.init();
    m = _m;
    n = _n;
  ...
  __device__ __forceinline__ void init_tile_pre(int col, int row) {
    ...
    // B_loading = B + (offset_k / PACK_RATIO_4BITS * n + offset_n) /
    // (FragBCount) + tv.slot_idx * n + (tv.wave_idx * SLOT +
    // tv.slot_tid)*N_ITERS;
    if constexpr (w_type_id == vllm::kU4.id() ||
                  w_type_id == vllm::kU4B8.id()) {
      B_loading =
          B + (offset_k / PACK_RATIO_4BITS * n + offset_n) / (FragBCount) +
          tv.slot_idx * n + (tv.wave_idx * SLOT + tv.slot_tid) * N_ITERS;
    } else if constexpr (w_type_id == vllm::kU8.id() ||
                         w_type_id == vllm::kU8B128.id()) {
      B_loading =
          B + (offset_k / PACK_RATIO_8BITS * n + offset_n) / (FragBCount) +
          tv.slot_idx * n * 2 + (tv.wave_idx * SLOT + tv.slot_tid) * N_ITERS;
    }
    scales_loading =
        scales + ((offset_k >> quant_group_power2) * n + offset_n) /
                     (sizeof(FragScaleLoading) / sizeof(scalar_t));
    if constexpr (w_type_id == vllm::kU4.id()) {
      zeros_loading =
          zeros +
          ((offset_k >> quant_group_power2) * n + offset_n) / PACK_RATIO_4BITS;
    }
  }
  ...
  __device__ __forceinline__ void init_bsm_addr() {
    bsm_a_ptr = (PackTypeInt4*)
        smem_base;  // use 8k bytes, will load at most 32x128*sizeof(half),
                    // either m32k128 or m128k32
    remaining_bsm_ptr = (float*)(smem_base + 0x2000 + 0x1000);  // 3k bytes
    bsm_a_ptr += tv.slot_tid *
                     (PAD_SLICE_K / (sizeof(PackTypeInt4) / sizeof(scalar_t))) +
                 tv.slot_idx;
    bsm_scales_ptr =
        (scalar_t*)(smem_base + 0x2000);  // use 128xsizeof(float)*2 = 1k
  ...
  __device__ __forceinline__ void write_c_pre() {
    ...
    int n_broad = (tv.wave_idx * SLOT + tv.slot_tid) * N_ITERS +
                  tile_manager.tile_start_col_cache * TILE_N;
#pragma unroll
    for (int miter = 0; miter < BLOCKS_M; miter++) {
#pragma unroll
      for (int niter = 0; niter < N_ITERS; niter++) {
...
#pragma unroll
  ...
  __device__ __forceinline__ void on_dequant(int kdx) {
    if constexpr (N_ITERS == 1)
      on_dequant_niters1<KTAIL>(kdx);
    else if constexpr (N_ITERS == 2)
      on_dequant_niters2<KTAIL>(kdx);
    else if constexpr (N_ITERS == 3)
      on_dequant_niters3<KTAIL>(kdx);
    else if constexpr (N_ITERS == 4)
      on_dequant_niters4<KTAIL>(kdx);
  }
};
...
template <typename scalar_t, const vllm::ScalarTypeId w_type_id,
          ...
          const bool HAS_M_PRED = true,  // If we should use predictors to load
                                         // m from gvm
          const bool HAS_NK_PRED = true  // If we should use predictors to load
                                         // nk from gvm
          >
__global__ void hgemm_gptq(
    const PackTypeInt4* __restrict__ A,  // fp16 input matrix of shape mxk
    const PackTypeInt4* __restrict__ B,  // 4bit quantized weight matrix of
                                         // shape kxn
    ...
    const PackTypeInt4* __restrict__ scales_ptr,  // fp16 quantization scales of
                                                  // shape (k/groupsize)xn
    const PackTypeInt4* __restrict__ zp_ptr,      // 4bit packed zero-points of
                                                  // shape
    ...
    int quant_group_power2,  // quant group means how many quanted values share
                             // the same scale and zero, this value restricts to
                             // 2^x where x >= 5
  ...
  using LoadingManagerType =
      LoadingManager<scalar_t, w_type_id, THREADS, BLOCKS_M, BLOCKS_N, BLOCKS_K,
                     HAS_ACT_ORDER, HAS_ZP, HAS_M_PRED, HAS_NK_PRED>;
  LoadingManagerType loading_manager;
  A += blockIdx.y * (MAX_BLOCKS_M * SLICE_M) * prob_k /
       (sizeof(PackTypeInt4) / sizeof(scalar_t));
  C += blockIdx.y * (MAX_BLOCKS_M * SLICE_M) * prob_n /
       (sizeof(PackTypeInt4) / sizeof(scalar_t));
#ifdef BF16_HIGH_PRECISION
  if constexpr (std::is_same_v<scalar_t, __maca_bfloat16>) {
    C_tmp += blockIdx.y * (MAX_BLOCKS_M * SLICE_M) * prob_n /
             (sizeof(PackTypeInt4) / sizeof(float));
  }
#endif
  loading_manager.set_address(A, B, C, C_tmp, scales_ptr, zp_ptr);
  // loading_manager.init_address(prob_m, prob_n, prob_k, bidx, max_iters,
  // smem_base);
  loading_manager.init_address_pre(
      std::min<int>(MAX_BLOCKS_M * SLICE_M,
                    prob_m - blockIdx.y * (MAX_BLOCKS_M * SLICE_M)),
      prob_n, prob_k, quant_group_power2, bidx, max_iters, smem_base);
  ...
  while (max_iters > 0) {
    ...
    loading_manager.ldg_a(
        0);  // Load first k0~31 and all m, one ldg_b128, heavy load
...
#pragma unroll BLOCKS_K - 1
      for (; k_idx < BLOCKS_K - 1; k_idx++) {
        ...
        // Loop for 3 times so that we can add some loading instructions before
        // matmul
...
#pragma unroll BLOCKS_M - 1
        ...
        barrier_bsm;  // wait for the last 16 batches ready, becasue of k_idx
                      // will be change
...
struct LoadingManager {
  ...
  constexpr static int DOUBLE_PAD_SLICE_K =
      SLICE_K * 2 + sizeof(PackTypeInt4) / sizeof(scalar_t);
  ...
  __device__ __forceinline__ void next_k0() {
    // reset bsm a to base
    bsm_a_ptr = (PackTypeInt4*)smem_base;
    bsm_a_ptr += tv.slot_tid * (DOUBLE_PAD_SLICE_K /
                                (sizeof(PackTypeInt4) / sizeof(scalar_t))) +
                 tv.slot_idx;
  }
...
#pragma unroll LOADING_A_LOOP
    for (int i = 0; i < LOADING_A_LOOP; i++) {
      ...
      if constexpr (HAS_M_PRED && HAS_NK_PRED) {
        ...
        ldg_b64_reg_noasync(temp_a[i], gvm_addr, tile_manager.global_pred,
                            true);
      }
      t += THREADS;
    }
...
#pragma unroll LOADING_A_LOOP
    for (int i = 0; i < LOADING_A_LOOP; i++) {
      ...
      int reading_k = t % (DOUBLE_SLICE_K / FragACount);
      int bsm_offset =
          reading_m * (DOUBLE_PAD_SLICE_K / FragACount) + reading_k;
      *(to_bsm_a_ptr + bsm_offset) = temp_a[i];
      t += THREADS;
    }
  }
  ...
  __device__ __forceinline__ void lds_a(int midx) {
    *((PackTypeInt4*)local_a[midx]) = *bsm_a_ptr;
    bsm_a_ptr +=
        SLOT * (DOUBLE_PAD_SLICE_K / (sizeof(PackTypeInt4) / sizeof(scalar_t)));
  }

  // TODO: implement when N_ITERS==1 or N_ITERS==3
  // korder used gptq_8bits, ldg_b will load two times in one SLICE_K
  // For example, t0 loads packed_k0, packed_k1, and packed_k0 represents a
  // packed 4 ks in first line of B, and packed_k1 represents a packed 4 ks in
  // second line of B
  __device__ __forceinline__ void ldg_b(int k_idx, int korder = 0) {
    if constexpr (HAS_NK_PRED) {
      bool pred_k = tile_manager.tile_start_row * TILE_K + k_idx * SLICE_K +
  ...
  __device__ __forceinline__ void sts_scales() {
    FragScaleLoading* scale_bsm =
        (FragScaleLoading*)(smem_base + 0x3000) + tv.tid;
    *scale_bsm = temp_scales;
  }
  ...
  __device__ __forceinline__ void sts_zeros() {
    if constexpr (w_type_id == vllm::kU4.id()) {
      ...
      if (pred) {
        ...
        decompress_zero_4bits(temp_zeros, temp);
        float* zeros_bsm =
            (float*)(smem_base + 0x3400) + tv.tid * PACK_RATIO_4BITS;
        for (int i = 0; i < PACK_RATIO_4BITS; i++) {
          *(zeros_bsm + i) = temp[i];
        }
      }
...
#pragma unroll
  ...
  __device__ __forceinline__ void init_bsm_addr() {
    ...
    bsm_a_ptr += tv.slot_tid * (DOUBLE_PAD_SLICE_K /
                 ...
                 tv.slot_idx;
    bsm_scales_ptr =
        (scalar_t*)(smem_base + 0x3000);  // use 128xsizeof(float)*2 = 1k
    bsm_scales_ptr += (tv.wave_idx * SLOT + tv.slot_tid) * N_ITERS;
    if constexpr (w_type_id == vllm::kU4.id()) {
      bsm_zeros_ptr =
          (float*)(smem_base + 0x3400);  // use 128xsizeof(float)*2 = 1k
      bsm_zeros_ptr += (tv.wave_idx * SLOT + tv.slot_tid) * N_ITERS;
    }
  }
...
#pragma unroll
  ...
  __device__ __forceinline__ void on_dequant(int kdx) {
    if constexpr (N_ITERS == 1)
      on_dequant_niters1<K, KTAIL>(kdx);
    else if constexpr (N_ITERS == 2)
      on_dequant_niters2<K, KTAIL>(kdx);
    else if constexpr (N_ITERS == 3)
      on_dequant_niters3<K, KTAIL>(kdx);
    else if constexpr (N_ITERS == 4)
      on_dequant_niters4<K, KTAIL>(kdx);
  }
};
...
#pragma unroll BLOCKS_M - 1
    ...
    barrier_bsm;  // wait for the last 16 batches ready, becasue of k_idx will
                  // be change
    loading_manager.next_k1();      // modify gvm/bsm address of a and b
    loading_manager.matmul(m_idx);  // do matmul
    m_idx = 0;
...
template <typename scalar_t, const vllm::ScalarTypeId w_type_id, int THREADS,
          int BLOCKS_M, int BLOCKS_N, int BLOCKS_K, bool HAS_ACT_ORDER,
          bool HAS_ZP, bool HAS_M_PRED, bool HAS_NK_PRED>
bool launch_gemm_gptq_kernel(const PackTypeInt4* A, const PackTypeInt4* B,
                             PackTypeInt4* C, PackTypeInt4* C_temp,
                             const PackTypeInt4* scales,
                             const PackTypeInt4* zeros, int* g_idx, int m,
                             int n, int k, int quant_group, int chunks,
  ...
  // printf("Launching hgemm_gptq_4bits THREADS=%d, BLOCKS_M=%d, BLOCKS_N=%d,
  // BLOCKS_K=%d, HAS_M_PRED=%d, HAS_NK_PRED=%d, tiles_n=%d, tiles_k = %d,
  // total_tiles = %d, iters = %d, blocks = %d, chunks = %d, TILE=m%dn%dk%d\n",
  // THREADS, BLOCKS_M, BLOCKS_N, BLOCKS_K, HAS_M_PRED, HAS_NK_PRED, tiles_n,
  // tiles_k, total_tiles, iters, blocks, chunks, BLOCKS_M*SLICE_M,
  // BLOCKS_N*SLICE_N, BLOCKS_K*SLICE_K
  // );

  if constexpr (std::is_same_v<scalar_t, __maca_bfloat16>) {
    size_t num_elem = size_t(m) * size_t(n);
    size_t clean_blocks = std::max(
        size_t(1),
        (num_elem + clean_kernel_thread_num * clean_kernel_pack_num - 1) /
            (clean_kernel_thread_num * clean_kernel_pack_num));
    clean_zero<clean_kernel_thread_num, clean_kernel_pack_num>
        <<<clean_blocks, clean_kernel_thread_num, 0, stream>>>((float*)C_temp,
                                                               num_elem);
  }

  // It is better to do perm before launch kernel
  if constexpr (BLOCKS_K % 2 == 1) {
    __hgemm_singular_blocks_k::hgemm_gptq<
        scalar_t, w_type_id, THREADS, BLOCKS_M, BLOCKS_N, BLOCKS_K,
        HAS_ACT_ORDER, HAS_ZP, HAS_M_PRED, HAS_NK_PRED>
        <<<dim3(std::max(blocks, 1), std::max(chunks, 1), 1), THREADS, 0,
           stream>>>(A, B, C, C_temp, scales, zeros, g_idx, m, n, k,
                     get_power2(quant_group), iters, nullptr, false);
  } else {
    __hgemm_even_blocks_k::hgemm_gptq<scalar_t, w_type_id, THREADS, BLOCKS_M,
                                      BLOCKS_N, BLOCKS_K, HAS_ACT_ORDER, HAS_ZP,
                                      HAS_M_PRED, HAS_NK_PRED>
        <<<dim3(std::max(blocks, 1), std::max(chunks, 1), 1), THREADS, 0,
           stream>>>(A, B, C, C_temp, scales, zeros, g_idx, m, n, k,
                     get_power2(quant_group), iters, nullptr, false);
  }
  ...
  if constexpr (std::is_same_v<scalar_t, __maca_bfloat16>) {
    size_t num_elem = size_t(m) * size_t(n);
    size_t reduce_blocks = std::max(
        size_t(1),
        (num_elem + reduce_kernel_thread_num * reduce_kernel_pack_num - 1) /
            (reduce_kernel_thread_num * reduce_kernel_pack_num));
    all_reduce<reduce_kernel_thread_num, reduce_kernel_pack_num, false>
        <<<reduce_blocks, reduce_kernel_thread_num, 0, stream>>>(
