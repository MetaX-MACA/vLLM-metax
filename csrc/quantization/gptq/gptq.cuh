...
#define LDG_ZEROS_4BITS                                                      \
  {                                                                          \
    ldg_b32_bsm_async(sts_zeros_offset, ldg_zeros_offset,                    \
                      tid < tileM / PACK_RATIO_4BITS &&                      \
                          (bidx * tileM + tid * PACK_RATIO_4BITS < align_m), \
                      false);                                                \
  }
...
#define LDG_ZEROS_8BITS                                                      \
  {                                                                          \
    ldg_b32_bsm_async(sts_zeros_offset, ldg_zeros_offset,                    \
                      tid < tileM / PACK_RATIO_8BITS &&                      \
                          (bidx * tileM + tid * PACK_RATIO_8BITS < align_m), \
                      false);                                                \
  }
...
#define PERM_ELEM_4BITS(index)                                            \
  {                                                                       \
    ...
    uint32_t zero =                                                       \
        __builtin_mxc_ubfe(rgZeros[0], Q4BITS * index, Q4BITS) + 1;       \
    if constexpr (doShuffle) {                                            \
      cast_half(rgA)[index * 4 + 0] = __hmul(                             \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 0, Q4BITS) - zero), \
          scale);                                                         \
      cast_half(rgA)[index * 4 + 1] = __hmul(                             \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 4, Q4BITS) - zero), \
          scale);                                                         \
      cast_half(rgA)[index * 4 + 2] = __hmul(                             \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 1, Q4BITS) - zero), \
          scale);                                                         \
      cast_half(rgA)[index * 4 + 3] = __hmul(                             \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 5, Q4BITS) - zero), \
          scale);                                                         \
      cast_half(rgA)[index * 4 + 32] = __hmul(                            \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 2, Q4BITS) - zero), \
          scale);                                                         \
      cast_half(rgA)[index * 4 + 33] = __hmul(                            \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 6, Q4BITS) - zero), \
          scale);                                                         \
      cast_half(rgA)[index * 4 + 34] = __hmul(                            \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 3, Q4BITS) - zero), \
          scale);                                                         \
      cast_half(rgA)[index * 4 + 35] = __hmul(                            \
          (__half)__builtin_mxc_i16_to_f16(                               \
              __builtin_mxc_ubfe(rga[index], Q4BITS * 7, Q4BITS) - zero), \
          scale);                                                         \
    } else {                                                              \
      cast_half(rgA)[index * 4 + 0] = __hmul(                             \
          (__half)__builtin_mxc_i16_to_f16(                               \
...
#define PERM_A1_4BITS \
  {PERM_ELEM_4BITS(0) PERM_ELEM_4BITS(1) PERM_ELEM_4BITS(2) PERM_ELEM_4BITS(3)}

#define PERM_A2_4BITS \
  {PERM_ELEM_4BITS(4) PERM_ELEM_4BITS(5) PERM_ELEM_4BITS(6) PERM_ELEM_4BITS(7)}
...
#define PERM_ELEM_8BITS(index)                                                 \
  {                                                                            \
    ...
    __half scale = __half(elem);                                               \
    uint32_t zero =                                                            \
        __builtin_mxc_ubfe(rgZeros[index / 4], Q8BITS * index, Q8BITS) + 1;    \
                                                                               \
    cast_half(rgA)[index * 4 + 0] =                                            \
        __hmul((__half)__builtin_mxc_i16_to_f16(                               \
                   __builtin_mxc_ubfe(rga[index], Q8BITS * 0, Q8BITS) - zero), \
               scale);                                                         \
    cast_half(rgA)[index * 4 + 1] =                                            \
        __hmul((__half)__builtin_mxc_i16_to_f16(                               \
                   __builtin_mxc_ubfe(rga[index], Q8BITS * 1, Q8BITS) - zero), \
               scale);                                                         \
    cast_half(rgA)[index * 4 + 2] =                                            \
        __hmul((__half)__builtin_mxc_i16_to_f16(                               \
                   __builtin_mxc_ubfe(rga[index], Q8BITS * 2, Q8BITS) - zero), \
               scale);                                                         \
    cast_half(rgA)[index * 4 + 3] =                                            \
        __hmul((__half)__builtin_mxc_i16_to_f16(                               \
                   __builtin_mxc_ubfe(rga[index], Q8BITS * 3, Q8BITS) - zero), \
               scale);                                                         \
    cast_half(rgA)[index * 4 + 32] = __hmul(                                   \
        (__half)__builtin_mxc_i16_to_f16(                                      \
            __builtin_mxc_ubfe(rga[index + 8], Q8BITS * 0, Q8BITS) - zero),    \
        scale);                                                                \
    cast_half(rgA)[index * 4 + 33] = __hmul(                                   \
        (__half)__builtin_mxc_i16_to_f16(                                      \
            __builtin_mxc_ubfe(rga[index + 8], Q8BITS * 1, Q8BITS) - zero),    \
        scale);                                                                \
    cast_half(rgA)[index * 4 + 34] = __hmul(                                   \
        (__half)__builtin_mxc_i16_to_f16(                                      \
            __builtin_mxc_ubfe(rga[index + 8], Q8BITS * 2, Q8BITS) - zero),    \
        scale);                                                                \
    cast_half(rgA)[index * 4 + 35] = __hmul(                                   \
        (__half)__builtin_mxc_i16_to_f16(                                      \
            __builtin_mxc_ubfe(rga[index + 8], Q8BITS * 3, Q8BITS) - zero),    \
        scale);                                                                \
  }

#define PERM_A1A3_8BITS \
  {PERM_ELEM_8BITS(0) PERM_ELEM_8BITS(1) PERM_ELEM_8BITS(2) PERM_ELEM_8BITS(3)}

#define PERM_A2A4_8BITS \
  {PERM_ELEM_8BITS(4) PERM_ELEM_8BITS(5) PERM_ELEM_8BITS(6) PERM_ELEM_8BITS(7)}
